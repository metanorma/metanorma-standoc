...
RSpec.describe Asciidoctor::Standoc::Datamodel::DiagramPreprocessor do
  describe '#process' do
    context 'when simple models without relations' do
      let(:datamodel_file) { examples_path('datamodel/common_models_diagram.adoc') }
      let(:result_file) { examples_path('datamodel/common_models_diagram.xml') }
      let(:output) do
        [BLANK_HDR, File.read(fixtures_path('macros_datamodel/common_models_diagram.xml'))]
          .join()
      end

      after do
        %w[doc html xml err].each do |extention|
          FileUtils.rm_f(examples_path("datamodel/common_models_diagram.#{extention}"))
    ...
    context 'when complex relations' do
      ...
      let(:output) do
        [BLANK_HDR, File.read(fixtures_path('macros_datamodel/top_down_diagram.xml'))]
          .join("\n")
      end

      after do
        %w[doc html xml err].each do |extention|
          FileUtils.rm_f(examples_path("datamodel/top_down_diagram.#{extention}"))
