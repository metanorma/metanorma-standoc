...
and key and value are both intended to be tokens (containing no punctuation). The `inherit`
attribute may be used to reference the label of a requirement that is imported or
presupposed by this requirement:

[source,asciidoctor]
--
[recommendation,label="/ogc/recommendation/wfs/2",subject="user",inherit="/ss/584/2015/level/1",classification="control-class:Technical;priority:P0;family:System and Communications Protection,System and Communications Protocols"]
====
I recommend this
====
--
